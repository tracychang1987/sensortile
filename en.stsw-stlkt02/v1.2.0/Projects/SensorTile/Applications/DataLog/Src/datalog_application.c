...
void DATALOG_SD_Init(void)
{
  ...
  if(FATFS_LinkDriver(&SD_Driver, SDPath) == 0)
  {
    ...
    if(f_mount(&SDFatFs, (TCHAR const*)SDPath, 0) != FR_OK)
    {
      ...
      while(1)
      {
       /// BSP_LED_On(LED1);/// by Chh170214
        HAL_Delay(500);
      ///  BSP_LED_Off(LED1);/// by Chh170214
        HAL_Delay(100);
      }
    }
  }
...
void Accelero_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
   ...
   ///printf("%x",acceleration.AXIS_X);
	  if(flag==0xaa)
	  {
		if(num==0)
		{
			sprintf( dataOut, "s:0%x\n", num );
		}
	  	if((acceleration.AXIS_X >= value) || (acceleration.AXIS_X <= -value) || (acceleration.AXIS_Y >= value) || (acceleration.AXIS_Y <= -value) ||(acceleration.AXIS_Z >= value) ||(acceleration.AXIS_Z <= -value))
	    {
	  		num++;
	...
	{
		flag=0xaa;
		if(double_click_num==0)
		{
			sprintf( dataOut, "d:0%x\n", double_click_num );
		}
		else if(double_click_num<=15)
		{
			sprintf( dataOut, "d:0%x\n", double_click_num );
		}
		else
...
void Pressure_Sensor_Handler( void *handle )
{
  ...
  if( status == 1 )
  {
    ...
    if(SendOverUSB)
    {
      if(d1<10)
      { 
	 	 sprintf(dataOut, "P:000%d.%02d", (int)d1, (int)d2);
      }
	  else if(d1<100)
	  {  
	  	sprintf(dataOut, "P:00%d.%02d", (int)d1, (int)d2);
	  }
	  else if(d1<1000)
	  { 
	 	 sprintf(dataOut, "P:0%d.%02d", (int)d1, (int)d2);
	  }
	  else if(d1<-1000)
	  {
		  sprintf(dataOut, "P:%d.%02d", (int)d1, (int)d2);
    ...
    {
      uint8_t size;
      size = sprintf( dataOut, "p:%5.2f\t", pressure);
      res = f_write(&MyFile, dataOut, size, (void *)&byteswritten);
    }
  }
}
